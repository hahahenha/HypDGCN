...
import manifolds
from layers.att_layers import GraphAttentionLayer, HypGraphAttentionLayer
import layers.hyp_layers as hyp_layers
from layers.rev_layers import RevLayer, OriRevLayer
from layers.self_layers import GraphConvolution, Linear, get_dim_act
...
class DeepGCN(Encoder):
    ...
    def __init__(self, c, args):
        ...
        dims, acts = get_dim_act(args)
        gc_layers = []
        gc_layers.append(GraphConvolution(dims[0], dims[1], args.dropout, acts[0], args.bias))
        gc_layers.append(OriRevLayer(dims, dims[0], dims[1], args.dropout, acts[0], args.bias))
        self.layers = nn.Sequential(*gc_layers)
        self.encode_graph = True
...
class HypDGCN(Encoder):
    """
    Hyperbolic & Deep GCN.
    """
    def __init__(self, c, args):
        super(HypDGCN, self).__init__(c)
        self.encode_graph = True
        self.manifold = getattr(manifolds, args.manifold)()
        assert args.num_layers > 1
        dims, acts, self.curvatures = hyp_layers.get_dim_act_curv(args)
        ...
        assert dims[1] % args.n_heads == 0
        concat = True

        layers = []
        layers.append(HypGraphAttentionLayer(self.curvatures[0], dims[0], dims[1] // args.n_heads, args.dropout, acts[0], args.alpha, args.n_heads, concat))
        layers.append(RevLayer(self.manifold, self.curvatures, dims, acts, args))
        self.layers = nn.Sequential(*layers)
    ...
    def encode(self, x, adj):
        ...
        x_hyp = self.manifold.expmap0(x_tan, c=self.curvatures[0])
        x_hyp = self.manifold.proj(x_hyp, c=self.curvatures[0])
        out = super(HypDGCN, self).encode(x_hyp, adj)
        return out
